"""Tests for RequestsCorrelation dataclass."""
...
class TestRequestsCorrelationFromContext:
    """Tests for RequestsCorrelation.from_context() class method."""
...
class TestRequestsCorrelationConstruction:
    """Tests for RequestsCorrelation construction and validation."""

    def test_construct_with_unsafe_chars_raises_value_error(self) -> None:
        """RequestsCorrelation constructor raises ValueError for unsafe chars."""
        with pytest.raises(ValueError, match=const.ERR_CORRELATION_ID_UNSAFE):
            objs.RequestsCorrelation(correlation_id="abc\r\n")

    def test_construct_with_empty_string_raises_value_error(self) -> None:
        """RequestsCorrelation constructor raises ValueError for empty string."""
        with pytest.raises(ValueError, match=const.ERR_CORRELATION_ID_UNSAFE):
            objs.RequestsCorrelation(correlation_id="")

    def test_construct_with_overlong_raises_value_error(self) -> None:
        """RequestsCorrelation constructor raises ValueError for overlong correlation_id."""
        overlong_id = "a" * 129
        with pytest.raises(ValueError, match=const.ERR_CORRELATION_ID_UNSAFE):
            objs.RequestsCorrelation(correlation_id=overlong_id)

    def test_construct_with_exactly_max_length_succeeds(self) -> None:
        """RequestsCorrelation constructor succeeds with exactly 128 chars."""
        exact_max_id = "a" * const.CORRELATION_ID_MAX_LENGTH
        correlation = objs.RequestsCorrelation(correlation_id=exact_max_id)
        assert correlation.correlation_id == exact_max_id


class TestRequestsCorrelationHeaderTuple:
    """Tests for RequestsCorrelation.header_tuple property."""
...
class TestRequestsCorrelationIsSafe:
    ...
    def test_is_safe_with_exactly_max_length_returns_true(self) -> None:
        ...
        assert objs.RequestsCorrelation._is_safe(exact_max) is True
    ...
    def test_is_safe_with_unsafe_chars_returns_false(self, unsafe_char: str) -> None:
        ...
        assert objs.RequestsCorrelation._is_safe(unsafe_value) is False
