"""Tests for HttpxCorrelation dataclass."""
...
class TestHttpxCorrelationFromContext:
    """Tests for HttpxCorrelation.from_context() class method."""
...
class TestHttpxCorrelationConstruction:
    """Tests for HttpxCorrelation construction and validation."""

    def test_construct_with_unsafe_chars_raises_value_error(self) -> None:
        """HttpxCorrelation constructor raises ValueError for unsafe chars."""
        with pytest.raises(ValueError, match=const.ERR_CORRELATION_ID_UNSAFE):
            objs.HttpxCorrelation(correlation_id="abc\r\n")

    def test_construct_with_empty_string_raises_value_error(self) -> None:
        """HttpxCorrelation constructor raises ValueError for empty string."""
        with pytest.raises(ValueError, match=const.ERR_CORRELATION_ID_UNSAFE):
            objs.HttpxCorrelation(correlation_id="")

    def test_construct_with_overlong_raises_value_error(self) -> None:
        """HttpxCorrelation constructor raises ValueError for overlong correlation_id."""
        overlong_id = "a" * 129
        with pytest.raises(ValueError, match=const.ERR_CORRELATION_ID_UNSAFE):
            objs.HttpxCorrelation(correlation_id=overlong_id)


class TestHttpxCorrelationHeaderTuple:
    """Tests for HttpxCorrelation.header_tuple property."""

    def test_header_tuple_returns_canonical_pair(self) -> None:
        """header_tuple returns (header_name, correlation_id) pair."""
        correlation = objs.HttpxCorrelation(correlation_id=test_const.HTTPX_CORR_ID_XYZ)
        name, value = correlation.header_tuple
        assert name == const.CORRELATION_ID_HEADER
        assert value == test_const.HTTPX_CORR_ID_XYZ


class TestHttpxCorrelationIsSafe:
    """Tests for HttpxCorrelation._is_safe() static method."""

    def test_is_safe_with_valid_value_returns_true(self) -> None:
        """_is_safe() returns True for valid correlation_id."""
        assert objs.HttpxCorrelation._is_safe(test_const.HTTPX_CORR_ID_SAFE) is True

    def test_is_safe_with_empty_returns_false(self) -> None:
        """_is_safe() returns False for empty string."""
        assert objs.HttpxCorrelation._is_safe("") is False

    def test_is_safe_with_overlong_returns_false(self) -> None:
        """_is_safe() returns False for string exceeding 128 chars."""
        overlong = "a" * 129
        assert objs.HttpxCorrelation._is_safe(overlong) is False
    ...
    def test_is_safe_with_unsafe_chars_returns_false(self, unsafe_char: str) -> None:
        ...
        assert objs.HttpxCorrelation._is_safe(unsafe_value) is False
