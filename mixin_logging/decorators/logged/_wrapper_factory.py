...
def wrap_callable(
    ...
    if for_static_or_class and class_module_name and class_name:
        ...
        if asyncio.iscoroutinefunction(method):
            ...
            async def async_static_wrapper(*args: Params.args, **kwargs: Params.kwargs) -> Result:
                ...
                if payload_from_request is not None:
                    try:
                        ...
                        if isinstance(extracted, dict):
                            ...
                            module_logger.warning("extraction.failure", extra={const.LOG_FIELD_ERROR_TYPE: "return_type_not_dict"})
                    except Exception as err:
                        module_logger.warning("extraction.failure", extra={const.LOG_FIELD_ERROR_TYPE: type(err).__name__})
                ...
                try:
                    ...
                    if timed and start_time is not None:
                        error_extra["latency_ms"] = (time.perf_counter() - start_time) * 1000
            ...
            if payload_from_request is not None:
                try:
                    ...
                    if isinstance(extracted, dict):
                        ...
                        instance.log_warning("extraction.failure", error_type="return_type_not_dict")
                except Exception as err:
                    instance.log_warning("extraction.failure", error_type=type(err).__name__)
                ...
                if payload_from_result is not None or timed:
                    ...
                    if payload_from_result is not None:
                        try:
                            ...
                            instance.log_warning("extraction.failure", error_type=type(err).__name__)
                    if timed and start_time is not None:
                        end_payload["latency_ms"] = (time.perf_counter() - start_time) * 1000
    ...
    @functools.wraps(method)
    def sync_instance_wrapper(instance: Service, *args: Params.args, **kwargs: Params.kwargs) -> Result:
        start_time = time.perf_counter() if timed else None
        start_payload = {}
        if payload_from_request is not None:
            try:
